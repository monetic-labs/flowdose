...
ssh -o StrictHostKeyChecking=no $SSH_USER@$IP_ADDRESS << ENDSSH
    ...
    cd /root/app/backend/.medusa/server && NODE_ENV=production NODE_TLS_REJECT_UNAUTHORIZED=0 pm2 start --name "medusa-server" yarn -- start
    cd /root/app/backend/.medusa/server && NODE_ENV=production NODE_TLS_REJECT_UNAUTHORIZED=0 pm2 start --name "medusa-worker" yarn -- start --worker
    ...
    for i in {1..5}; do
        if curl -s http://localhost:9000/health | grep -q "OK"; then
            ...
            if [ $i -eq 5 ]; then
                echo "⚠️ Warning: Server health check failed, but deployment continuing."
            else
