...
if [ "$CI_MODE" = true ]; then
    ...
    # Determine the correct path to the validation script
    if [ -f "./validate-storefront-env.sh" ]; then
        VALIDATE_SCRIPT="./validate-storefront-env.sh"
    elif [ -f "../scripts/validate-storefront-env.sh" ]; then
        VALIDATE_SCRIPT="../scripts/validate-storefront-env.sh"
    else
        echo "Error: Cannot find validate-storefront-env.sh script."
        exit 1
    fi
